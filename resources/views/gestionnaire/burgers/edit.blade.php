...
<body class="h-full">
    <div class="d-flex">
        ...
        <div class="flex-grow-1 p-4">
            ...
            <!-- Formulaire pour modifier un burger -->
            <div class="card">
                <div class="card-header">
                    <h4>Modifier {{ $burger->name }}</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('burgers.update', $burger) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $burger->name) }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="price" class="form-label">Prix (FCFA)</label>
                            <input type="number" name="price" id="price" class="form-control" value="{{ old('price', $burger->price) }}" step="0.01" required>
                        </div>
                        <div class="mb-3">
                            <label for="stock" class="form-label">Stock</label>
                            <input type="number" name="stock" id="stock" class="form-control" value="{{ old('stock', $burger->stock) }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="category" class="form-label">Catégorie</label>
                            <input type="text" name="category" id="category" class="form-control" value="{{ old('category', $burger->category) }}">
                        </div>
                        <div class="mb-3">
                            <label for="description" class="form-label">Description</label>
                            <textarea name="description" id="description" class="form-control">{{ old('description', $burger->description) }}</textarea>
                        </div>
                        <div class="mb-3">
                            ...
                            @if ($burger->image)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $burger->image) }}" alt="{{ $burger->name }}" style="width: 100px; height: 100px; object-fit: cover; border-radius: 5px;">
                                </div>
                            @endif
                        </div>
                        <div class="mb-3">
                            <label for="archived" class="form-label">Archivé</label>
                            <select name="archived" id="archived" class="form-control">
                                <option value="0" {{ !$burger->archived ? 'selected' : '' }}>Non</option>
                                <option value="1" {{ $burger->archived ? 'selected' : '' }}>Oui</option>
                            </select>
                        </div>
